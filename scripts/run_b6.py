...
"""
B6 阶段主入口脚本
GRU 版本的 Warm-Start 专家模型训练 + Hard vs Soft 路由对比

与 B5.5 的区别：
- B5.5: 使用 LSTM 作为基座模型
- B6: 使用 GRU 作为基座模型，验证 GRU 专家是否能进一步提升性能

流程：
1. 复用 B3 的 Router 组件 (scaler_g, pca, fcm_centers)
2. 检查 model_global_gru.pt 是否存在，不存在则先训练 Global GRU
3. Warm-start: 加载 model_global_gru.pt 初始化每个 GRU 专家
4. 微调专家模型 (按簇 train/val)
5. Hard vs Soft rolling evaluation
6. 输出 metrics_B6.json/csv
"""
...
# =============================================================================
# 状态向量 g_t 构建 (与 B3/B4/B5/B5.5 完全一致)
# =============================================================================
...
# =============================================================================
# Global GRU 训练 (类似 B1，但使用 GRU)
# =============================================================================

@dataclass
class GlobalGRUTrainResult:
    best_epoch: int
    best_val_loss: float
    train_samples: int
    val_samples: int
...
def train_global_gru(
    ...
    patience: int = 10,
    seed: int = 42
) -> Tuple[GRUModel, GlobalGRUTrainResult]:
    """
    训练 Global GRU 模型 (作为专家的 warm-start 基座)
    """
    ...
    print(f"\n  [Global GRU] Train: {len(X_train)}, Val: {len(X_val)}, LR: {lr:.6f}")
...
def train_expert_gru_warm(
    ...
    patience: int = 10,
    seed: int = 42
) -> Tuple[GRUModel, ExpertTrainResult]:
    """
    Warm-start 训练单个 GRU 专家模型
    
    Args:
        global_model_path: Global GRU 模型路径
        lr_scale_finetune: 微调学习率缩放因子
    """
    input_size = X_train.shape[2]
    output_steps = y_train.shape[1]
    
    # 创建 GRU 模型 (与 Global GRU 结构一致)
    model = GRUModel(
        input_size=input_size,
        hidden_size=hidden_size,
        num_layers=num_layers,
    ...
    # Warm-start: 加载 Global GRU 权重
    if global_model_path.exists():
        global_state = torch.load(global_model_path, map_location=device, weights_only=True)
        model.load_state_dict(global_state)
        print(f"  [Warm-start] Loaded Global GRU from {global_model_path.name}")
    else:
        print(f"  [Warning] Global GRU not found: {global_model_path}, training from scratch")
    ...
    print(f"\n  [GRU Expert {cluster_id}] Train: {len(X_train)}, Val: {len(X_val)}, LR: {finetune_lr:.6f}")
    print(f"   {'Epoch':>5} | {'Train Loss':>12} | {'Val Loss':>12} | {'4h RMSE(pu)':>12}")
    print(f"  {'-'*50}")
    ...
    if best_state is not None:
        model.load_state_dict(best_state)
        print(f"  [Checkpoint] Best epoch weights loaded (epoch={best_epoch})")
...
def main():
    print("\n" + "=" * 70)
    print("B6 阶段 - GRU Warm-Start 专家模型 + Hard vs Soft 路由对比")
    print("=" * 70)
    ...
    routing_mode = soft_routing_config.get('routing_mode', 'hard')  # B6 默认 hard
    ...
    if routing_mode not in ('hard', 'soft'):
        print(f"  [Warning] Invalid routing_mode '{routing_mode}', defaulting to 'hard'")
        routing_mode = 'hard'
    
    # Warm-start 配置 (B6 使用 GRU)
    ...
    print(f"  - Base model: GRU (vs B5.5 LSTM)")
    ...
    # =========================================================================
    # 7.5 检查/训练 Global GRU (warm-start 基座)
    # =========================================================================
    global_gru_path = results_dir / "model_global_gru.pt"
    ...
    if not global_gru_path.exists():
        print(f"\n[7.5/10] Global GRU 不存在，开始训练...")
        print("=" * 60)
        print("Training Global GRU (warm-start base)")
        print("=" * 60)
        
        global_gru, global_gru_result = train_global_gru(
            X_train=X_train_all,
            y_train=y_train_all,
            X_val=X_val_all,
            y_val=y_val_all,
        ...
        # 保存 Global GRU
        torch.save(global_gru.state_dict(), global_gru_path)
        print(f"  Saved: {global_gru_path.name}")
    else:
        print(f"\n[7.5/10] Global GRU 已存在: {global_gru_path.name}")
    
    # =========================================================================
    # 8. Warm-Start 训练 GRU 专家模型
    # =========================================================================
    print("\n[8/10] Warm-Start 训练 GRU 专家模型...")
    ...
    for k in range(n_clusters):
        print(f"\n{'='*60}")
        print(f"Training GRU Expert {k} ({cluster_semantic_map.get(k, 'unknown')})")
        print(f"{'='*60}")
        ...
        model, result = train_expert_gru_warm(
            cluster_id=k,
            X_train=X_train_k,
            y_train=y_train_k,
            X_val=X_val_k,
            y_val=y_val_k,
            global_model_path=global_gru_path,
            device=device,
            hidden_size=hidden_size,
            num_layers=num_layers,
            dropout=dropout,
    ...
    # 打印汇总表
    print("\n" + "=" * 80)
    print(f"B6 汇总表 - GRU Warm-Start 专家模型 (routing_mode={routing_mode.upper()})")
    print("=" * 80)
    ...
    # 保存 GRU 专家模型
    for k in range(n_clusters):
        model_path = results_dir / f"model_expert_{k}_gru.pt"
    ...
    metrics_data = {
        ...
        'config': {
            ...
            'warm_start_path': 'model_global_gru.pt',
            'lr_scale_finetune': lr_scale_finetune,
            'routing_mode': routing_mode,
    ...
    json_path = results_dir / "metrics_B6.json"
    with open(json_path, 'w', encoding='utf-8') as f:
        json.dump(metrics_data, f, indent=2, ensure_ascii=False)
    print(f"  Saved: {json_path.name}")
    
    # 保存 CSV
    csv_path = results_dir / "metrics_B6.csv"
    ...
    log_path = logs_dir / f"train_log_b6_{timestamp}.json"
    log_data = {
        'timestamp': timestamp,
        'stage': 'B6',
        'base_model': 'GRU',
